...
const systemSettings = [
 ...
 { name:'Access Keys', icon: Key, href:'/settings/access-keys' },
...
export default function Sidebar() {
 ...
 const filteredCore = corePlatform.filter(item => authUser ? hasPermission(authUser.role, item.href) : true);
 const filteredFinancial = financialModules.filter(item => authUser ? hasPermission(authUser.role, item.href) : true);
 const filteredSystem = systemSettings.filter(item => authUser ? hasPermission(authUser.role, item.href) : true);
...
"bg-muted border-border flex flex-col overflow-hidden",
 ...
 <div className={cn(
"flex items-center p-6 px-8 justify-start transition-all duration-300",
 ...
 {/* Sidebar Nav */}
 <div className={cn(
"flex-1 overflow-y-auto px-4 py-2 space-y-2 pb-10 transition-all duration-500",
...
"mt-auto border-t border-border p-6 transition-all duration-500",
 ...
 getInitials(authUser?.name || profileUser?.name || "")
 )}
