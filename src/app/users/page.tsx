    ...
    <div className="space-y-8 pb-20 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          <Button variant="outline" className="h-10 rounded-xl font-black text-[10px] uppercase tracking-widest bg-card border-border/40 hover:bg-secondary flex items-center gap-2">
            <Download size={14} /> Export Report
          </Button>
          <Button className="h-10 rounded-xl font-black text-[10px] uppercase tracking-widest bg-primary text-white shadow-lg shadow-primary/20 flex items-center gap-2">
      ...
      <div className="bg-card border border-border/40 rounded-[28px] p-4 flex flex-col xl:flex-row items-center gap-4 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full xl:w-auto">
          {["All Users", "KYC Level", "Risk Score", "Device Status"].map((f, i) => (
            <button key={i} className="flex-1 xl:flex-none flex items-center justify-between gap-3 px-4 py-2 bg-secondary/40 border border-border/20 rounded-xl text-[10px] font-black uppercase tracking-widest text-muted-foreground hover:bg-primary/5 hover:text-primary hover:border-primary/20 transition-all">
              {f} <ChevronDown size={12} className="text-muted-foreground/40" />
            </button>
          ))}
          <Button variant="ghost" className="h-9 w-9 p-0 rounded-xl border border-border/20 hover:bg-secondary">
      ...
      <div className="bg-card border border-border/40 rounded-[32px] overflow-hidden shadow-sm flex flex-col">
        <div className="px-8 py-5 border-b border-border/10 bg-secondary/10 flex items-center justify-between">
           ...
           <div className="flex bg-muted/50 p-1 rounded-xl border border-border/20">
              {['all', 'flagged', 'suspended'].map((tab) => (
                <button 
                  key={tab} 
                  onClick={() => setActiveTab(tab)}
                  className={cn(
                    "px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all",
                    activeTab === tab ? "bg-primary text-white shadow-lg" : "text-muted-foreground hover:text-foreground"
                  )}
                >
                  {tab}
                </button>
          ...
          <>
            ...
            >
              ...
              <div className="p-8 border-t border-border/20 bg-secondary/10 grid grid-cols-2 gap-4">
                 <div className="space-y-2">
                    <Button className="w-full h-12 bg-red-500 hover:bg-red-600 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-xl shadow-red-500/20 flex items-center justify-center gap-2">
                       <LockIcon size={14} /> Freeze Account
                    </Button>
                    <Button variant="outline" className="w-full h-12 border-red-500/20 text-red-500 hover:bg-red-500/5 rounded-2xl font-black text-[11px] uppercase tracking-widest flex items-center justify-center gap-2">
                       <UserX size={14} /> Suspend Access
                    </Button>
                 </div>
                 <div className="space-y-2">
                    <Button className="w-full h-12 bg-primary text-white rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-xl shadow-primary/20 flex items-center justify-center gap-2">
