...
const Badge = ({ status }: { status: string }) => {
 ...
 frozen:'bg-rose-500/10 text-rose-600 border-rose-500/20',
...
export default function SavingsOperationsPage() {
 ...
 console.log('Executing handleExportCSV');
 setIsExporting('CSV');
 await executeExport({
 fileName:'Savings_Ledger',
 data: filteredData.map(plan => ({
 'ID': plan.id,
 'Customer': plan.user.name,
 'Plan': plan.plan.name,
 'Type': plan.plan.type,
 ...
 console.log('Executing handleGenerateReport');
 setIsExporting('PDF');
 
 const totalBalance = filteredData.reduce((acc, curr) => acc + curr.balance, 0);
 ...
 await executeExport({
 fileName:'Savings_Growth_Report',
 summary: [
 `Total Savings Accounts: ${filteredData.length}`,
 `Combined Savings Balance: ₦${totalBalance.toLocaleString()}`,
 `Aggregate Target Goal: ₦${totalTarget.toLocaleString()}`,
 `Overall Progress: ${avgProgress}%`
 ],
 data: filteredData.map(plan => ({
 'Customer': plan.user.name,
 'Plan': plan.plan.name,
 'Balance': `₦${plan.balance.toLocaleString()}`,
 'Target': `₦${plan.target.toLocaleString()}`,
 'Progress': `${totalTarget > 0 ? Math.min(Math.round((plan.balance / plan.target) * 100), 100) : 0}%`,
 'Status': plan.status
 })),
 format:'PDF'
 });
 ...
 variant="outline" 
 size="sm" 
 className="h-9 rounded-xl border-border font-bold text-muted-foreground bg-card shadow-sm flex items-center gap-2 hover:bg-secondary hover:text-foreground cursor-pointer opacity-100"
 >
 {isExporting ==='CSV' ? <RefreshCw size={14} className="animate-spin" /> : <Download size={14} />} 
 Export CSV
 </Button>
 <Button 
 onClick={() => {
 ...
 className="h-9 rounded-xl border-border font-bold text-muted-foreground bg-card shadow-sm flex items-center gap-2 hover:bg-secondary hover:text-foreground cursor-pointer opacity-100"
 >
 {isExporting ==='PDF' ? <RefreshCw size={14} className="animate-spin" /> : <FileText size={14} />} 
 Generate Report
 </Button>
 </div>
 </div>
 ...
 <td className="px-4 py-2.5">
 <div className="text-[12px] font-black text-emerald-600">{formatCurrency(plan.contribution)}</div>
